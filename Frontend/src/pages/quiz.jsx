...
const Quiz=()=>{    
  ...
  const submitQuiz = async () => {
    ...
    const scoresjson = await evaluateQuiz(user.token, selectedTopic.id, answers)
    setScore(scoresjson)
    ...
    if (scoresjson.score >= 80 && selectedTopic.progress < 2) {
      ...
      if (progress.message === "Ok") {
          ...
          selectedTopic = {...selectedTopic, progress: 2}
      }
    }
    localStorage.setItem('triggerQuiz', `Quiz Score: ${scoresjson.score}`)
  }
